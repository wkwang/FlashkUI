	...
	public class List extends UIComponent
	{
		...
		protected function resetScrollBar():void
		{
			if(items.numChildren>0)
			{
				...
				for(var i:int=0;i<items.numChildren;i++){
					items.getChildAt(i).y = IListItemRender(render).itemHeight*i;
					items.getChildAt(i).visible = true;
					IListItemRender(items.getChildAt(i)).index = i;
				}
		...
		{
//			if(_isSizeInit == true && newWidth == _compoWidth && newHeight == _compoHeight) return;
