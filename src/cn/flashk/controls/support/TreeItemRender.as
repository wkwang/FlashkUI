package cn.flashk.controls.support
{
	...
	public class TreeItemRender extends ListItemRender
	{
		...
		protected override  function showOver(event:MouseEvent=null):void
		{
			...
			if(_selected == false){
				bg.alpha = StyleManager.treeOverAlpha;
			}
		}
		...
		public function checkOpenNode(node:Object,childNodes:Object = null):void{
			if(_data == node){
				...
				for(var i:int=0;i<opens.length;i++){
					TreeItemRender(opens[i]).checkOpenNode(node,childNodes);
				}
			}
		}
...
//				}
		...
		private function openNode(event:MouseEvent = null,myChildNodes:Object = null):void{
			if(items == null)
			{
				items = new Sprite();
				this.addChild(items);
			...
			isOpen = !isOpen;
			Tree(_list).setOpenState(_data,isOpen);
...
//			trace("open:",getTimer()-t,isOpen,lenRef,len,"creat:",creatCount);
		}
